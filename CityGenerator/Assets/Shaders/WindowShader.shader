Shader "Custom/WindowAtlasShader" {
      Properties {
        ...
        _DoorTexture ("Door Texture", 2D) = "white" {} // Новое свойство для текстуры двери
        _DoorTextureTiling ("Door Texture Tiling", Vector) = (1, 1, 0, 0) // Tiling для текстуры двери
        _DoorTextureOffset ("Door Texture Offset", Vector) = (0, 0, 0, 0) // Offset для текстуры двери
    }
    ...
    float doorPixelValue = tex2D(_WindowPositionMap, i.uv).g; // Значение пикселя в канале G
    ...
    if (doorPixelValue != 0.0) { // Если значение в канале G не равно 0, считаем, что это дверь
        float2 doorTextureUV = i.uv * _DoorTextureTiling.xy + _DoorTextureOffset.xy;
